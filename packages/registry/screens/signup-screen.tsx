 ...
 * SignUpScreen
 *
 * Complete sign up screen with name, email, password fields,
 * terms checkbox, and social signup options.
 *
 * @example
 * ```tsx
 * <SignUpScreen
 *   onSignUp={(data) => createAccount(data)}
...
export interface SignUpScreenProps {
  ...
  /** Called when social sign up button is pressed */
  onSocialSignUp?: (provider: SocialProvider) => void | Promise<void>;
  /** Called when login link is pressed */
  onLogin?: () => void;
  /** Called when terms link is pressed */
  onTermsPress?: () => void;
  /** Called when privacy link is pressed */
  onPrivacyPress?: () => void;
  /** Show loading state on sign up button */
  loading?: boolean;
...
export function SignUpScreen({
  ...
  const handleSignUp = () => {
    onSignUp?.({ name, email, password, acceptedTerms });
  };

  const isValid = name && email && password && acceptedTerms;
    ...
    >
      ...
      >
        ...
        <View style={[styles.form, { gap: spacing[4] }]}>
          ...
          {/* Terms Checkbox */}
          <View style={[styles.termsContainer, { marginTop: spacing[2] }]}>
            <Checkbox
