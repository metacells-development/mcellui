...
Import tokens from packages/core/src/theme/typography.ts:
```typescript
...
- input.tsx (line ~382): fontWeight: '500' -> fontWeight.medium
- textarea.tsx (line ~259): fontWeight: '500' -> fontWeight.medium
- select.tsx (lines ~291, 302, 316, 319, 320):
  - fontWeight: '500' -> fontWeight.medium
  - fontSize: 10 -> fontSize['2xs']
  - fontSize: 16 -> fontSize.md
  - fontWeight: '600' -> fontWeight.semibold
- stepper.tsx (lines ~293, 305, 309):
  - fontWeight: '500' -> fontWeight.medium
  - fontWeight: '600' -> fontWeight.semibold
  - fontWeight: '400' -> fontWeight.normal
- checkbox.tsx (lines ~250, 253):
  - fontWeight: '500' -> fontWeight.medium
  - fontSize: 13 -> fontSize.sm (round down, 12 is close to 13)
...
Files and specific changes:
- toast.tsx (lines ~306, 307, 310, 314, 315):
  - fontSize: 14 -> fontSize.base
  - fontWeight: '600' -> fontWeight.semibold
  - fontSize: 13 -> fontSize.sm
- toggle.tsx (lines ~111, 118, 125, 381):
  - fontSize: 12 -> fontSize.sm
  - fontSize: 14 -> fontSize.base
  - fontSize: 16 -> fontSize.md
  - fontWeight: '500' -> fontWeight.medium
- circular-progress.tsx (line ~260):
  - fontWeight: '600' -> fontWeight.semibold
- tag-input.tsx (lines ~500, 507, 522, 525):
  - fontWeight: '500' -> fontWeight.medium
  - fontSize: 12 -> fontSize.sm
- alert.tsx (line ~292):
  - fontWeight: '600' -> fontWeight.semibold
