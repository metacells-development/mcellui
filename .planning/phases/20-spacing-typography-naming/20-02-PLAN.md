...
Search each file for hardcoded borderRadius values in StyleSheet.create() blocks:

Per-file targets:
- input.tsx: Find `borderRadius: 4` -> radius.sm
- chip.tsx: Find `borderRadius: 4` -> radius.sm
- alert.tsx: Find `borderRadius: 4` -> radius.sm
- action-sheet.tsx: Find `borderRadius: 2` -> radius.xs
...
Search each file for hardcoded borderRadius values:

Per-file targets:
- image-gallery.tsx: Find `borderRadius: 1` -> Keep as 1 (too small for token, intentional subtle rounding)
- calendar.tsx: Find `borderRadius: 2` -> radius.xs
- datetime-picker.tsx: Find all `borderRadius` values. Replace `borderRadius: 2` -> radius.xs. Keep `borderRadius: 1` and `borderRadius: 1.5` as-is (intentional sub-token design details).
