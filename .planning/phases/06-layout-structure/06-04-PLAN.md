...
must_haves:
  truths:
    - "Row, Column, and Screen components already use tokens correctly (verified)"
...
<objective>
Verify Row, Column, Screen components use tokens correctly and migrate their demo files

Purpose: First verify that the actual Row, Column, and Screen components already use spacing and color tokens correctly (per research findings). Then replace hardcoded styling values in demo files with theme tokens for consistent appearance across themes and proper dark mode support.

Output: Verified component token compliance + three demo files using tokens instead of hardcoded values
</objective>
...
<task type="auto">
  <name>Task 0: Verify Row, Column, Screen components use tokens correctly</name>
  <files>packages/registry/ui/row.tsx, packages/registry/ui/column.tsx, packages/registry/ui/screen.tsx</files>
  <action>
Verify that the actual layout components already use tokens correctly:

1. **Row component** (row.tsx):
   - Verify `semanticGapMap` maps to spacing key indices (not hardcoded pixels)
   - Verify `resolveGap()` looks up `spacing[key]` (not hardcoded values)
   - Verify no hardcoded colors

2. **Column component** (column.tsx):
   - Same as Row - verify semantic gap mapping uses spacing tokens
   - Verify no hardcoded colors

3. **Screen component** (screen.tsx):
   - Verify `paddedStyle` uses `spacing[4]` for paddingHorizontal
   - Verify variant backgrounds use `colors.background` / `colors.card`
   - Verify no hardcoded colors or spacing values

Research confirmed these components are compliant. This task documents verification.
  </action>
  <verify>
```bash
cd /Users/johanneschristler/Code/metacells/other/mcellui
# Row: Check semanticGapMap uses indices, not pixels
grep -A8 "semanticGapMap" packages/registry/ui/row.tsx
# Row: Check resolveGap uses spacing lookup
grep -A6 "resolveGap" packages/registry/ui/row.tsx | head -10
# Screen: Check padded uses spacing token
grep "paddingHorizontal" packages/registry/ui/screen.tsx
# Check no hardcoded hex colors in components
grep -E "'#[0-9a-fA-F]{6}'" packages/registry/ui/row.tsx packages/registry/ui/column.tsx packages/registry/ui/screen.tsx || echo "No hardcoded colors found"
```
  </verify>
  <done>
Row, Column, and Screen components verified to use spacing and color tokens correctly. semanticGapMap references spacing indices, resolveGap looks up spacing[key], Screen uses spacing[4] for padded prop, and no hardcoded hex colors found. Components are compliant - only demos need migration.
  </done>
</task>
...
<verification>
1. Row, Column, Screen COMPONENTS use tokens correctly (Task 0 verification)
2. All three demos import and use shared Section from './section'
3. No '#737373' hex color in any file
4. No hardcoded fontSize values (12, 14, 16, etc.)
5. No hardcoded gap values (8, 12, 16, 24)
6. All borderRadius use radius tokens
7. TypeScript compiles without errors
</verification>

<success_criteria>
- Row, Column, Screen components verified to use spacing/color tokens
